(function () {
    ...
    // UI
    w = new Window("dialog", "Select JPEG Folder");
    ...
    btnFolderInput.onClick = function () {
        var f = Folder.selectDialog("Select a folder containing JPEG files");
    ...
    btnOk.onClick = function () {
        if (selectedFolder) {
            var jpegFiles = selectedFolder.getFiles(function (file) {
            ...
            if (jpegFiles.length > 0) {
                for (var i = 0; i < jpegFiles.length; i++) {
                    var newFileName = selectedFolder.fullName + "/" + (i + 1) + ".jpeg";
                    jpegFiles[i].rename(newFileName); // Rename the file
                }
                alert("Renamed " + jpegFiles.length + " JPEG files.");
            } else {
                alert("No JPEG files found in the selected folder.");
            }
        } else {
